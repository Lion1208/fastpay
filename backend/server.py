...
class AdminUserUpdate(BaseModel):
    ...
    taxa_saque_depix: Optional[float] = None
...
async def recalculate_user_balance(user_id: str):
    ...
    total_recebido = sum(
        t.get("valor_liquido") if t.get("valor_liquido") and t.get("valor_liquido") > 0 
        else t.get("valor", 0) 
...
async def subscribe_push(data: PushSubscription, user: dict = Depends(get_current_user)):
    ...
    if existing:
        return {"message": "Subscription já registrada"}
